...
Independent H100 / CPU benchmark for `AnalyticBandRadiation.jl` against
...
- `Project.toml` — environment. `[sources]` path-deps onto this repo's
  `AnalyticBandRadiation.jl` and the developing Breeze checkout at
...
RADIATIVE_HEATING_ECCKD_LW_PATH=$(julia --project=. -e 'using AnalyticBandRadiation; print(official_ecckd_definition_path(:longwave_32))') \
RADIATIVE_HEATING_ECCKD_SW_PATH=$(julia --project=. -e 'using AnalyticBandRadiation; print(official_ecckd_definition_path(:shortwave_32))') \
