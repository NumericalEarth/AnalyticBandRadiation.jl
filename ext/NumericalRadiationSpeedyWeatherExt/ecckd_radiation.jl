# EcCKDRadiation: clear-sky ecCKD gas optics with longwave and shortwave solved from one
# gas-optics evaluation, as a single SpeedyWeather `radiation` component.

"""
    default_ozone_profile(p)

Ozone mole fraction as a function of pressure `p` [Pa]: a Chapman-layer shape
peaking at 8 ppmv near 30 hPa and vanishing towards the surface. A crude
stand-in until SpeedyWeather carries an ozone field; pass your own
`ozone = p -> ...` or a per-column constant to [`EcCKDRadiation`](@ref).
"""
...
"""
    EcCKDRadiation{NF} <: SpeedyWeather.AbstractRadiation

Clear-sky ecCKD radiation (longwave and shortwave from one gas-optics
evaluation) as a single SpeedyWeather `radiation` component. Gas optical
properties are interpolated from the tabulated ecCKD model for every column
and both streams are solved with NumericalRadiation's clear-sky solvers.

Gases: `composite` (dry air) and `h2o` follow from surface pressure and specific
humidity, `co2` from the model's `greenhouse_gases` (`co2`, in ppm) or
`default_CO₂`, every other gas of the ecCKD model from `mole_fractions`
(a number, or a function of pressure [Pa]); `ozone` is a shortcut for
`mole_fractions.o3`. Gases the ecCKD model knows but that are not requested
when loading it are held at their reference concentration.

```julia
using SpeedyWeather, NumericalRadiation, NCDatasets
spectral_grid = SpectralGrid(truncation = 32, nlayers = 8)
radiation = EcCKDRadiation(spectral_grid, "32x32")
model = PrimitiveWetModel(spectral_grid; radiation)
```
"""
struct EcCKDRadiation{NF, GO, MF} <: SpeedyWeather.AbstractRadiation
    "Tabulated ecCKD gas optics in the grid's number format"
    gas_optics::GO
    "CO₂ concentration [ppm] used when the model has no `greenhouse_gases.co2`"
    default_CO₂::NF
    "Mole fractions of the remaining gases, numbers or functions of pressure [Pa]"
    mole_fractions::MF
    "Longwave emissivity over ocean [1]"
    emissivity_ocean::NF
...
function EcCKDRadiation(SG::SpeedyWeather.SpectralGrid, gas_optics::EcCKDTabulatedGasOpticsModel;
                        CO₂ = 280,
    ...
    for name in NumericalRadiation.gas_names(gas_optics)
        name in (:composite, :h2o, :co2) && continue
...
"""$(TYPEDSIGNATURES)
Load the reference ecCKD `model_pair` (e.g. `"32x32"`, `"64x96"`) with the gases
`names` and wrap it into an [`EcCKDRadiation`](@ref). Needs `NCDatasets` loaded."""
EcCKDRadiation(SG::SpeedyWeather.SpectralGrid, model_pair::Union{AbstractString, Symbol};
               names = (:composite, :h2o, :o3, :co2), kwargs...) =
    EcCKDRadiation(SG, read_reference_ecckd_gas_optics(model_pair; names); kwargs...)

EcCKDRadiation(SG::SpeedyWeather.SpectralGrid; kwargs...) = EcCKDRadiation(SG, "32x32"; kwargs...)

Base.show(io::IO, rad::EcCKDRadiation) = show(io, rad, values = false)

SpeedyWeather.initialize!(::EcCKDRadiation, ::SpeedyWeather.PrimitiveEquation) = nothing

number_of_gases(rad::EcCKDRadiation) = length(NumericalRadiation.gas_names(rad.gas_optics))

function SpeedyWeather.variables(rad::EcCKDRadiation{NF}, model::SpeedyWeather.AbstractModel) where NF
...
@inline gas_amount(::Val{:composite}, rad, dry, h2o, x_co2, p) = dry
@inline gas_amount(::Val{:h2o}, rad, dry, h2o, x_co2, p) = h2o
@inline gas_amount(::Val{:co2}, rad, dry, h2o, x_co2, p) = x_co2 * dry
@inline gas_amount(::Val{name}, rad, dry, h2o, x_co2, p) where name =
...
solar zenith angle."""
Base.@propagate_inbounds function ecckd_surface_state(ij, vars, rad::EcCKDRadiation, model)
...
Base.@propagate_inbounds function ecckd_column_optics(ij, W, rad::EcCKDRadiation)
    (; gas_optics) = rad
...
Base.@propagate_inbounds function ecckd_longwave!(ij, vars, fluxes, longwave, atmosphere,
                                                  rad::EcCKDRadiation{NF}, surface) where NF
    (; gas_optics) = rad
    W = vars.parameterizations.ecckd
...
Base.@propagate_inbounds function ecckd_shortwave!(ij, vars, fluxes, shortwave, atmosphere,
                                                   rad::EcCKDRadiation{NF}, surface, model) where NF
    W = vars.parameterizations.ecckd
    nlayers = length(atmosphere.temperature_layers)
