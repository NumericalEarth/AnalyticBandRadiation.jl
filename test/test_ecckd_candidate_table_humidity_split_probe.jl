...
module EcckdCandidateTableHumiditySplitProbeValidation
include(joinpath(@__DIR__, "..", "validation",
                 "ecckd_candidate_table_humidity_split_probe.jl"))
end

@testset "ecCKD candidate table humidity-split probe" begin
    mktempdir() do root
        sw_path = joinpath(root, expected_flux_path("ckdmip_evaluation1_sw_fluxes_rel-415.h5"))
        write_sw_flux_fixture(sw_path)
        reference =
            EcckdCandidateTableHumiditySplitProbeValidation.official_ecckd_definition_path(
                :shortwave_32,
            )
        candidate = joinpath(root, "humidity_split_candidate.nc")

        result =
            EcckdCandidateTableHumiditySplitProbeValidation.run_table_humidity_split_probe(;
                root,
        ...
        @test result.tested_move_count == 4
        ...
        markdown =
            EcckdCandidateTableHumiditySplitProbeValidation.markdown_table_humidity_split_probe(
                result,
            )
        @test occursin("ecCKD Candidate Table Humidity-Split Probe", markdown)
