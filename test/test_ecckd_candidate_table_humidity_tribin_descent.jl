...
module EcckdCandidateTableHumidityTribinDescentValidation
include(joinpath(@__DIR__, "..", "validation",
                 "ecckd_candidate_table_humidity_tribin_descent.jl"))
end

@testset "ecCKD candidate table humidity-tribin descent" begin
    mktempdir() do root
        sw_path = joinpath(root, expected_flux_path("ckdmip_evaluation1_sw_fluxes_rel-415.h5"))
        write_sw_flux_fixture(sw_path)
        reference =
            EcckdCandidateTableHumidityTribinDescentValidation.official_ecckd_definition_path(
                :shortwave_32,
            )
        candidate = joinpath(root, "humidity_tribin_descent_candidate.nc")

        result =
            EcckdCandidateTableHumidityTribinDescentValidation.run_table_humidity_tribin_descent(;
                root,
        ...
        markdown =
            EcckdCandidateTableHumidityTribinDescentValidation.markdown_table_humidity_tribin_descent(
                result,
            )
        @test occursin("ecCKD Candidate Table Humidity-Tribin Descent", markdown)
