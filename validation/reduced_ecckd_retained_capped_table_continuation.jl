...
function with_retained_capped_table_continuation_environment(f)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_CAPPED_TABLE_CONTINUATION",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_CONSTRAINED_TABLE_SCOPE"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_SCOPE", "all_global_residual_probe")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_RESIDUAL_MODE"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_RESIDUAL_MODE", "surface")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_CANDIDATES"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_CANDIDATES", "48")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_PROBE_POOL_MULTIPLIER"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_PROBE_POOL_MULTIPLIER", "2")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_PROBE_STEP"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_PROBE_STEP", "0.0009765625")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_MAX_LOG_SCALE"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_MAX_LOG_SCALE", "0.00390625")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_INCLUDE_RAYLEIGH"] =
            get(ENV, "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_INCLUDE_RAYLEIGH", "true")
        ENV["RH_REDUCED_CONSTRAINED_TABLE_RIDGE_LAMBDAS"] =
            get(
                ENV,
                "RH_REDUCED_RETAINED_CAPPED_TABLE_CONTINUATION_RIDGE_LAMBDAS",
                "1.0e-8,1.0e-6,1.0e-4,1.0e-2,1.0,100.0,1.0e4,1.0e6",
            )
        ENV["RH_REDUCED_IGNORE_RETAINED_CAPPED_TABLE_CONTINUATION"] = "true"
        return f()
    finally
        for key in keys
            if old[key] === nothing
...
function retained_capped_table_continuation_result()
    surface_cap = retained_capped_table_surface_cap()
    toa_tolerance = retained_capped_table_toa_tolerance()
    base_capped_moves = latest_retained_capped_table_optimizer_moves()
    raw = with_retained_capped_table_continuation_environment() do
        constrained_table_optimizer_result(extra_base_moves = base_capped_moves)
    end
    ...
    return merge(
        raw,
        (
            case = "reduced_ecckd_retained_capped_table_continuation",
            status = accepted ? "retained_capped_table_continuation_improved" :
                     "retained_capped_table_continuation_rejected",
            timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
            method = "bounded-frontier retained capped table continuation with absolute surface cap",
            surface_cap_w_m2 = surface_cap,
            toa_tolerance_w_m2 = toa_tolerance,
            base_capped_move_count = length(base_capped_moves),
...
function retained_capped_table_continuation_markdown(result)
    lines = String[
        "# Reduced ecCKD Retained Capped Table Continuation",
        "",
        ...
        "| Base capped moves | $(result.base_capped_move_count) |",
...
function main(; result = nothing)
    result = result === nothing ? retained_capped_table_continuation_result() : result
    mkpath(dirname(RETAINED_CAPPED_TABLE_CONTINUATION_JSON))
    write(RETAINED_CAPPED_TABLE_CONTINUATION_JSON, json_object(result) * "\n")
    write(RETAINED_CAPPED_TABLE_CONTINUATION_MD,
          retained_capped_table_continuation_markdown(result))
    print(retained_capped_table_continuation_markdown(result))
    println("Wrote $RETAINED_CAPPED_TABLE_CONTINUATION_JSON")
    println("Wrote $RETAINED_CAPPED_TABLE_CONTINUATION_MD")
end
