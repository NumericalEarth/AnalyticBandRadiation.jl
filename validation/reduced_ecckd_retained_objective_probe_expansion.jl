...
const RETAINED_OBJECTIVE_PROBE_EXPANSION_JSON =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_objective_probe_expansion.json")
const RETAINED_OBJECTIVE_PROBE_EXPANSION_MD =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_objective_probe_expansion.md")

function with_objective_probe_env(f; candidates, probe_step, max_log_scale)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_OBJECTIVE_PROBE_EXPANSION",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_IGNORE_RETAINED_OBJECTIVE_PROBE_EXPANSION"] = "true"
        return f()
    finally
...
function retained_objective_probe_expansion_result()
    configs = (
        (
            label = "objective_probe_24_medium_tiny",
            candidates = 24,
            probe_step = 0.000244140625,
            max_log_scale = 0.001953125,
        ),
        (
            label = "objective_probe_24_small_tiny",
            candidates = 24,
    ...
    for config in configs
        result = with_objective_probe_env(
    ...
    return (
        case = "reduced_ecckd_retained_objective_probe_expansion",
        timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
        status = accepted ? "objective_probe_expansion_improved" :
...
function retained_objective_probe_expansion_markdown(result)
    lines = String[
...
function main(; result = nothing)
    result = result === nothing ? retained_objective_probe_expansion_result() :
        result
    mkpath(dirname(RETAINED_OBJECTIVE_PROBE_EXPANSION_JSON))
    write(RETAINED_OBJECTIVE_PROBE_EXPANSION_JSON, json_object(result) * "\n")
    write(RETAINED_OBJECTIVE_PROBE_EXPANSION_MD,
          retained_objective_probe_expansion_markdown(result))
    print(retained_objective_probe_expansion_markdown(result))
    println("Wrote $RETAINED_OBJECTIVE_PROBE_EXPANSION_JSON")
    println("Wrote $RETAINED_OBJECTIVE_PROBE_EXPANSION_MD")
end
