...
const RETAINED_OBJECTIVE_PROBE_EXPANSION2_JSON =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_objective_probe_expansion2.json")
const RETAINED_OBJECTIVE_PROBE_EXPANSION2_MD =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_objective_probe_expansion2.md")

function with_objective_probe2_env(f; candidates, probe_step, max_log_scale)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_OBJECTIVE_PROBE_EXPANSION2",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_IGNORE_RETAINED_OBJECTIVE_PROBE_EXPANSION2"] = "true"
        return f()
    finally
...
function retained_objective_probe_expansion2_result()
    configs = (
        (
            label = "objective_probe2_24_medium_tiny",
            candidates = 24,
            probe_step = 0.000244140625,
            max_log_scale = 0.001953125,
        ),
        (
            label = "objective_probe2_24_small_tiny",
            candidates = 24,
    ...
    for config in configs
        result = with_objective_probe2_env(
    ...
    return (
        case = "reduced_ecckd_retained_objective_probe_expansion2",
        timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
        status = accepted ? "objective_probe_expansion2_improved" :
...
function retained_objective_probe_expansion2_markdown(result)
    lines = String[
        "# Reduced ecCKD Retained Objective-Probe Expansion 2",
        "",
        "Status: **$(result.status)**",
        "",
        "| Field | Value |",
        ...
        "the first accepted objective-probe expansion has been promoted into the",
        "canonical retained reduced model, accepting only strict Pareto-safe",
...
function main(; result = nothing)
    result = result === nothing ? retained_objective_probe_expansion2_result() :
        result
    mkpath(dirname(RETAINED_OBJECTIVE_PROBE_EXPANSION2_JSON))
    write(RETAINED_OBJECTIVE_PROBE_EXPANSION2_JSON, json_object(result) * "\n")
    write(RETAINED_OBJECTIVE_PROBE_EXPANSION2_MD,
          retained_objective_probe_expansion2_markdown(result))
    print(retained_objective_probe_expansion2_markdown(result))
    println("Wrote $RETAINED_OBJECTIVE_PROBE_EXPANSION2_JSON")
    println("Wrote $RETAINED_OBJECTIVE_PROBE_EXPANSION2_MD")
end
