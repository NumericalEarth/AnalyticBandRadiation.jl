...
const RETAINED_OBJECTIVE_PROBE_EXPANSION4_JSON =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_objective_probe_expansion4.json")
const RETAINED_OBJECTIVE_PROBE_EXPANSION4_MD =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_objective_probe_expansion4.md")

function with_objective_probe4_env(f; candidates, probe_step, max_log_scale)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_OBJECTIVE_PROBE_EXPANSION4",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_IGNORE_RETAINED_OBJECTIVE_PROBE_EXPANSION4"] = "true"
        return f()
    finally
...
function retained_objective_probe_expansion4_result()
    configs = (
        (
            label = "objective_probe4_32_medium_tiny",
            candidates = 32,
            probe_step = 0.000244140625,
            max_log_scale = 0.001953125,
        ),
        (
            label = "objective_probe4_32_small_tiny",
            candidates = 32,
    ...
    for config in configs
        result = with_objective_probe4_env(
    ...
    return (
        case = "reduced_ecckd_retained_objective_probe_expansion4",
        timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
        status = accepted ? "objective_probe_expansion4_improved" :
...
function retained_objective_probe_expansion4_markdown(result)
    lines = String[
        "# Reduced ecCKD Retained Objective-Probe Expansion 4",
        "",
        "Status: **$(result.status)**",
        "",
        "| Field | Value |",
        ...
        "the first three accepted objective-probe expansions have been promoted,",
...
function main(; result = nothing)
    result = result === nothing ? retained_objective_probe_expansion4_result() :
        result
    mkpath(dirname(RETAINED_OBJECTIVE_PROBE_EXPANSION4_JSON))
    write(RETAINED_OBJECTIVE_PROBE_EXPANSION4_JSON, json_object(result) * "\n")
    write(RETAINED_OBJECTIVE_PROBE_EXPANSION4_MD,
          retained_objective_probe_expansion4_markdown(result))
    print(retained_objective_probe_expansion4_markdown(result))
    println("Wrote $RETAINED_OBJECTIVE_PROBE_EXPANSION4_JSON")
    println("Wrote $RETAINED_OBJECTIVE_PROBE_EXPANSION4_MD")
end
