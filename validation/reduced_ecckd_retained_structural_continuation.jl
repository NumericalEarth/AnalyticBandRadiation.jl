...
function retained_structural_continuation_result()
    set_retained_structural_continuation_default!(
        "RH_REDUCED_CONSTRAINED_TABLE_BASE_MODE",
        "boundary_table_continuation",
    )
    set_retained_structural_continuation_default!(
        "RH_REDUCED_IGNORE_RETAINED_STRUCTURAL_CONTINUATION",
        "true",
    )
    set_retained_structural_continuation_default!(
...
function retained_structural_continuation_markdown(result)
    lines = String[
...
function main(; result = nothing)
    result = result === nothing ? retained_structural_continuation_result() :
        result
    mkpath(dirname(RETAINED_STRUCTURAL_CONTINUATION_JSON))
    write(RETAINED_STRUCTURAL_CONTINUATION_JSON, json_object(result) * "\n")
    write(RETAINED_STRUCTURAL_CONTINUATION_MD,
          retained_structural_continuation_markdown(result))
    print(retained_structural_continuation_markdown(result))
    println("Wrote $RETAINED_STRUCTURAL_CONTINUATION_JSON")
    println("Wrote $RETAINED_STRUCTURAL_CONTINUATION_MD")
end
