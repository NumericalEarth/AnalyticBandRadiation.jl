...
const RETAINED_STRUCTURAL_OPTIMIZER_JSON =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_structural_optimizer.json")
const RETAINED_STRUCTURAL_OPTIMIZER_MD =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_structural_optimizer.md")

function set_retained_structural_default!(key, value)
    haskey(ENV, key) || (ENV[key] = value)
    return ENV[key]
end

function retained_structural_optimizer_result()
    set_retained_structural_default!(
        "RH_REDUCED_CONSTRAINED_TABLE_BASE_MODE",
        "boundary_table_continuation",
    )
    set_retained_structural_default!(
        "RH_REDUCED_IGNORE_RETAINED_STRUCTURAL_OPTIMIZER",
        "true",
    )
    set_retained_structural_default!(
...
function main(; result = nothing)
    result = result === nothing ? retained_structural_optimizer_result() : result
    mkpath(dirname(RETAINED_STRUCTURAL_OPTIMIZER_JSON))
    write(RETAINED_STRUCTURAL_OPTIMIZER_JSON, json_object(result) * "\n")
    write(RETAINED_STRUCTURAL_OPTIMIZER_MD, retained_structural_markdown(result))
    print(retained_structural_markdown(result))
    println("Wrote $RETAINED_STRUCTURAL_OPTIMIZER_JSON")
    println("Wrote $RETAINED_STRUCTURAL_OPTIMIZER_MD")
end
