...
function with_surface_probe_env(f; candidates, probe_step, max_log_scale)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_SURFACE_PROBE_EXPANSION",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_IGNORE_RETAINED_SURFACE_PROBE_EXPANSION"] = "true"
        return f()
    finally
...
function retained_surface_probe_expansion_result()
    configs = (
        (
            label = "surface_probe_32_medium_tiny",
            candidates = 32,
            probe_step = 0.000244140625,
            max_log_scale = 0.001953125,
        ),
        (
            label = "surface_probe_32_small_tiny",
            candidates = 32,
    ...
    return (
        case = "reduced_ecckd_retained_surface_probe_expansion",
        timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
        status = accepted ? "surface_probe_expansion_improved" :
...
function retained_surface_probe_expansion_markdown(result)
    lines = String[
...
function main(; result = nothing)
    result = result === nothing ? retained_surface_probe_expansion_result() :
        result
    mkpath(dirname(RETAINED_SURFACE_PROBE_EXPANSION_JSON))
    write(RETAINED_SURFACE_PROBE_EXPANSION_JSON, json_object(result) * "\n")
    write(RETAINED_SURFACE_PROBE_EXPANSION_MD,
          retained_surface_probe_expansion_markdown(result))
    print(retained_surface_probe_expansion_markdown(result))
    println("Wrote $RETAINED_SURFACE_PROBE_EXPANSION_JSON")
    println("Wrote $RETAINED_SURFACE_PROBE_EXPANSION_MD")
end
