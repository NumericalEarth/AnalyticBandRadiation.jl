...
const RETAINED_SURFACE_PROBE_EXPANSION3_JSON =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_surface_probe_expansion3.json")
const RETAINED_SURFACE_PROBE_EXPANSION3_MD =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_surface_probe_expansion3.md")

function with_surface_probe3_env(f; candidates, probe_step, max_log_scale)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_SURFACE_PROBE_EXPANSION3",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_IGNORE_RETAINED_SURFACE_PROBE_EXPANSION3"] = "true"
        return f()
    finally
...
function retained_surface_probe_expansion3_result()
    configs = (
        (
            label = "surface_probe3_32_medium_tiny",
            candidates = 32,
            probe_step = 0.000244140625,
            max_log_scale = 0.001953125,
        ),
        (
            label = "surface_probe3_32_small_tiny",
            candidates = 32,
    ...
    for config in configs
        result = with_surface_probe3_env(
    ...
    return (
        case = "reduced_ecckd_retained_surface_probe_expansion3",
        timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
        status = accepted ? "surface_probe_expansion3_improved" :
...
function retained_surface_probe_expansion3_markdown(result)
    lines = String[
...
function main(; result = nothing)
    result = result === nothing ? retained_surface_probe_expansion3_result() :
        result
    mkpath(dirname(RETAINED_SURFACE_PROBE_EXPANSION3_JSON))
    write(RETAINED_SURFACE_PROBE_EXPANSION3_JSON, json_object(result) * "\n")
    write(RETAINED_SURFACE_PROBE_EXPANSION3_MD,
          retained_surface_probe_expansion3_markdown(result))
    print(retained_surface_probe_expansion3_markdown(result))
    println("Wrote $RETAINED_SURFACE_PROBE_EXPANSION3_JSON")
    println("Wrote $RETAINED_SURFACE_PROBE_EXPANSION3_MD")
end
