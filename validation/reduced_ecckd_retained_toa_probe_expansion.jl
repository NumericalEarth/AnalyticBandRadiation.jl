...
const RETAINED_TOA_PROBE_EXPANSION_JSON =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_toa_probe_expansion.json")
const RETAINED_TOA_PROBE_EXPANSION_MD =
    joinpath(@__DIR__, "results", "reduced_ecckd_retained_toa_probe_expansion.md")

function with_toa_probe_env(f; candidates, probe_step, max_log_scale)
    keys = (
        ...
        "RH_REDUCED_IGNORE_RETAINED_TOA_PROBE_EXPANSION",
    )
    ...
    try
        ...
        ENV["RH_REDUCED_IGNORE_RETAINED_TOA_PROBE_EXPANSION"] = "true"
        return f()
    finally
...
function retained_toa_probe_expansion_result()
    configs = (
        (
            label = "toa_probe_32_medium_tiny",
            candidates = 32,
            probe_step = 0.000244140625,
            max_log_scale = 0.001953125,
        ),
        (
            label = "toa_probe_32_small_tiny",
            candidates = 32,
    ...
    return (
        case = "reduced_ecckd_retained_toa_probe_expansion",
        timestamp_utc = Dates.format(now(UTC), dateformat"yyyy-mm-ddTHH:MM:SS.sss"),
        status = accepted ? "toa_probe_expansion_improved" :
                 "toa_probe_expansion_rejected",
...
function retained_toa_probe_expansion_markdown(result)
    lines = String[
...
function main(; result = nothing)
    result = result === nothing ? retained_toa_probe_expansion_result() :
        result
    mkpath(dirname(RETAINED_TOA_PROBE_EXPANSION_JSON))
    write(RETAINED_TOA_PROBE_EXPANSION_JSON, json_object(result) * "\n")
    write(RETAINED_TOA_PROBE_EXPANSION_MD,
          retained_toa_probe_expansion_markdown(result))
    print(retained_toa_probe_expansion_markdown(result))
    println("Wrote $RETAINED_TOA_PROBE_EXPANSION_JSON")
    println("Wrote $RETAINED_TOA_PROBE_EXPANSION_MD")
end
